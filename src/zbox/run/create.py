...
def main_argv(argv: list[str]) -> None:
    ...
    print_color(f"Creating zbox container named '{box_name}'", fg=fgcolor.green)
    if verify_zbox_state(docker_cmd, box_name, [], exit_on_error=False):
        print_error(f"Zbox container '{box_name}' already exists.")
        sys.exit(1)
    ...
    setup_zbox_scripts(conf, distro_config)
    ...
    print_info("Waiting for the container to initialize (see "
               f"'zbox-logs -f {box_name}' for detailed progress)")
    sys.stdout.flush()
...
def run_shared_copy_container(docker_cmd: str, image_name: str, shared_root_dirs: str,
    ...
    if os.path.exists(root_host):
        input_msg = f"""
            The shared root directory for '{conf.distribution}' already exists in:
                {root_host}
            However, the corresponding zbox container image for '{conf.distribution}' does not
        ...
        if response.lower() == "y":
            ...
            print_error(f"Aborting creation of zbox container '{conf.box_name}'")
    ...
    run_command([docker_cmd, "run", "-it", f"--name={conf.box_name}",
                 ...
                 f"--label={ZboxLabel.CONTAINER_COPY}", "--userns=keep-id", "--user=0",
...
def start_container(docker_full_cmd: list[str], current_user: str, shared_root_dirs: str,
    ...
    docker_full_cmd.append(f"--label={ZboxLabel.CONTAINER_DISTRIBUTION}={conf.distribution}")
    ...
    if (code := int(run_command(docker_full_cmd, exit_on_error=False,
        ...
        print_error(f"Also check 'zbox-logs {conf.box_name}' for details")
        sys.exit(code)
...
def wait_for_container(docker_cmd: str, conf: StaticConfiguration) -> None:
    ...
    with open(conf.status_file, "r", encoding="utf-8") as status_fd:
        for _ in range(max_wait_secs):
            ...
            if verify_zbox_state(docker_cmd, box_name, ["running"], exit_on_error=False):
                ...
                print_error("FAILED waiting for container to be ready -- check "
                            f"'zbox-logs {box_name}' for details")
                sys.exit(1)
    ...
    print_error(f"TIMED OUT waiting for ready container after {max_wait_secs}secs -- check "
                f"'zbox-logs -f {box_name}' for details")
    sys.exit(1)
...
def restart_container(docker_cmd: str, conf: StaticConfiguration) -> None:
    if (code := int(run_command([docker_cmd, "container", "start", conf.box_name],
        ...
        print_error("Also check 'zbox-logs {conf.box_name}' for details")
        sys.exit(code)
